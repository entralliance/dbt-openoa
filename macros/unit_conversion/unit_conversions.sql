...
{% macro entr_single_tag_unit_conversion(relation_for_table_structure, entr_tag_id, operation, new_units, cte=relation_for_table_structure, where='1=1') %}
    {{ return(adapter.dispatch('entr_single_tag_unit_conversion', 'openoa')(relation_for_table_structure, entr_tag_id, operation, new_units, cte, where)) }}
{% endmacro %}

{% macro default__entr_single_tag_unit_conversion(relation_for_table_structure, entr_tag_id, operation, new_units, cte=relation_for_table_structure, where='1=1') %}

    {% set columns = dbt_utils.get_filtered_columns_in_relation(from=relation_for_table_structure) %}
    select
...
{% macro entr_multiple_tag_unit_conversions(relation_for_table_structure, entr_tag_ids, operations, new_units, cte=relation_for_table_structure, where='1=1') %}
    {{ return(adapter.dispatch('entr_multiple_tag_unit_conversions', 'openoa')(relation_for_table_structure, entr_tag_ids, operations, new_units, cte, where)) }}
{% endmacro %}

{% macro default__entr_multiple_tag_unit_conversions(relation_for_table_structure, entr_tag_ids, operations, new_units, cte=relation_for_table_structure, where='1=1') %}
